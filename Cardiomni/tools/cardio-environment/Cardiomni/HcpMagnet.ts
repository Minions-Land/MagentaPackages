 ...
 * cardio-environment process descriptor (UV); the Client-owned builder creates
 * the host-backed Tool product from the relocatable descriptor.
 */
export class HcpMagnet {
	static readonly module = "tools/cardio-environment";
	...
	static async build(context: HcpMagnetBuildContext) {
		const products = await HcpMagnetbuildtools(context, {
			kind: "tool",
			name: "cardio-environment",
			source: "Cardiomni",
			descriptorPath: join(dirname(fileURLToPath(import.meta.url)), "cardio-environment.toml"),
		});
