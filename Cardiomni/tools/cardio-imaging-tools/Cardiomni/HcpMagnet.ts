...
export class HcpMagnet {
	static readonly module = "tools/cardio-imaging-tools";
	...
	static async build(context: HcpMagnetBuildContext) {
		const products = await HcpMagnetbuildtools(context, {
			kind: "tool",
			name: "cardio-imaging-tools",
			source: "Cardiomni",
			descriptorPath: join(dirname(fileURLToPath(import.meta.url)), "cardio-imaging-tools.toml"),
		});
